...
int main(int argc, char* argv[]) {      
    ...
    if (bufSize < 2 || bufSize > 500) { //checks buffer size 2-500
        printf("Buffer size should be 2-500.\n");
        exit(1);
    }
    if (itemCnt < 1) { //checks number of items to be positive
        printf("The number of items produced/consumed must be greater than 0.\n");
        exit(1);
    }
...
void InitShm(int bufSize, int itemCnt){
    ...
    const char *name = "OS_HW1_ChrisWright"; // Name of shared memory object to be passed to shm_open
    ...
    ftruncate(fd, SHM_SIZE);  //configure size of shared memory object
...
void Producer(int bufSize, int itemCnt, int randSeed) {
    ...
    srand(randSeed);  //seed random function
    ...
    int i = 0;
    for (i; i < itemCnt; i++) {
        while (((in + 1) % bufSize) == GetOut());  //wait until buffer has space
        int val = GetRand(5, 2500);                //assign random value
        WriteAtBufIndex(in, val);                  //update shared memory buffer
        printf("Producing Item %d with Value %d at Index %d\n", i, val, in);
        in = (in + 1) % bufSize;                   //update in
        SetIn(in);                                 //update header in
    }
